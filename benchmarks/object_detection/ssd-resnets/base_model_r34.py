...
class ResNet34(nn.Module):
    def __init__(self, tiles_path, sparsity_ratio, simulation_file):
        super().__init__()
        rn34 = resnet34(tiles_path, sparsity_ratio, simulation_file, pretrained=True)
...
def make_layers(cfg, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            layers += [nn.SimulatedMaxPool2d(kernel_size=2, stride=2)]
        elif v == 'C':
            # Notice ceil_mode is true
            layers += [nn.SimulatedMaxPool2d(kernel_size=2, stride=2, ceil_mode=True)]
        else:
