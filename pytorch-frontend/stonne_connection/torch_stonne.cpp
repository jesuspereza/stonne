...
torch::Tensor simulated_avg_pool_forward(std::string layer_name, torch::Tensor input, c10::ArrayRef<long int> kernel_size, c10::ArrayRef<long int> stride, c10::ArrayRef<long int> padding, c10::ArrayRef<long int> dilation, std::string path_to_arch_file, std::string path_to_tile) {
    //Here starts the function
    //Creating config file to find out if we are going to run a dense or sparse simulation
    Config stonne_cfg; 
    if(path_to_arch_file != "") {
    ...
    simulateAvgPoolingForward(layer_name, input_raw, output_raw, R, S, C, N, X, Y, H_out, W_out, strides, pad_x, pad_y, path_to_tile, stonne_cfg);
    return output;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("simulated_max_pool_forward", &simulated_max_pool_forward, "Simulated max pool forward");
  m.def("simulated_avg_pool_forward", &simulated_avg_pool_forward, "Simulated avg pool forward");
