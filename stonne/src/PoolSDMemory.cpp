...
unsigned int VNATPool_Register::get_address() {
    return this->base_addr + addr;
}


//TODO revisar esta funcion
void VNATPool_Register::update() {
    this->current_S+=1;
...
PoolSDMemory::PoolSDMemory(id_t id, std::string name, Config stonne_cfg, Connection* write_connection) : MemoryController(id, name) {
...
void PoolSDMemory::setWriteConnections(std::vector<Connection*> write_port_connections) {
...
void PoolSDMemory::setReadConnections(std::vector<Connection*> read_connections) {
...
void PoolSDMemory::setTile(Tile* current_tile) {
    ...
    this->VNAT = new VNATPool_Register*[num_vn];
            ...
            for(unsigned k=0; k<this->current_tile->get_T_K(); k++) {
                for(unsigned x=0; x<this->current_tile->get_T_X_(); x++) {
                    for(unsigned y=0; y<this->current_tile->get_T_Y_();y++) {
                        ...
                        this->VNAT[current_vn]=new VNATPool_Register(current_vn, addr_offset, n, g, k, x, y, iter_N, iter_G, iter_K, iter_X, iter_Y, VNAT_iter_R, VNAT_iter_S, VNAT_iter_C, this->dnn_layer, this->current_tile);
                    }
                }
            } //End K
        } //End G
...
void PoolSDMemory::setLayer(DNNLayer* dnn_layer, address_t input_address, address_t filter_address, address_t output_address, Dataflow dataflow) {
    this->dnn_layer = dnn_layer;
...
bool PoolSDMemory::isExecutionFinished() {
...
void PoolSDMemory::sendPackageToInputFifos(DataPackage* pck) {
    // BROADCAST PACKAGE
    if(pck->isBroadcast()) {
...
void PoolSDMemory::receive() { //TODO control if there is no space in queue
...
void PoolSDMemory::printStats(std::ofstream& out, unsigned int indent) {
...
void PoolSDMemory::printEnergy(std::ofstream& out, unsigned int indent) {
    /*
